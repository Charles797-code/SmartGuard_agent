...
from src.modules.input_handler import TextInputHandler, AudioInputHandler, VisualInputHandler, VisualInput, AudioInput
...
async def lifespan(app: FastAPI):
    ...
    local_model_path = r"D:\agent\models\models--shibing624--text2vec-base-chinese"
...
# 确定前端文件路径（相对于项目根目录）
# __file__ = agent/src/api/main.py (相对路径)
# abspath 后 = D:\new\agentnew\agentnew\agent\src\api\main.py
# 3x dirname 后 = D:\new\agentnew\agentnew\agent  (项目根目录)
_project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_frontend_dist = os.path.join(_project_root, "web", "dist")

# 如果 dist 不存在，则使用 web 目录（开发模式）
if not os.path.exists(_frontend_dist):
    _frontend_src = os.path.join(_project_root, "web")
else:
    _frontend_src = _frontend_dist

# 挂载静态文件目录（优先级低于 API 路由）
if os.path.exists(_frontend_src):
    app.mount("/static", StaticFiles(directory=_frontend_src, html=True), name="static")


@app.get("/", response_class=HTMLResponse)
async def serve_index():
    """根路径返回前端页面"""
    # 优先查找 dist/index.html
    dist_index = os.path.join(_frontend_dist, "index.html")
    src_index = os.path.join(_frontend_src, "index.html")

    if os.path.exists(dist_index):
        with open(dist_index, "r", encoding="utf-8") as f:
            return f.read()
    elif os.path.exists(src_index):
        ...
        <!DOCTYPE html>
        <html>
        <head><meta charset="utf-8"><title>SmartGuard</title></head>
        <body style="font-family:sans-serif;background:#0f172a;color:#f1f5f9;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0;">
            <div style="text-align:center;">
                <h1>🛡️ SmartGuard API</h1>
                <p>服务正在运行，请访问 /docs 查看 API 文档</p>
                <a href="/docs" style="color:#818cf8;">API 文档</a>
            </div>
        </body>
        </html>
        """
...
async def serve_mobile():
    """手机版页面"""
    mobile_path = os.path.join(_project_root, "web", "mobile.html")
...
async def analyze_multimodal(
    ...
    if audio:
        try:
            ...
            print(f"[DEBUG] Remote audio: content_type={audio.content_type}, size={len(audio_bytes)}, first_bytes={audio_bytes[:8].hex()}")
            audio_input = AudioInput(
                audio_data=audio_bytes,
                metadata={"mime_type": audio.content_type or "audio/mp3"}
            )
...
async def chat(
    ...
    if image:
        try:
            ...
            print(f"[ERROR] [Chat API] Image processing failed: {e}", flush=True)
    ...
    if audio:
        try:
            ...
            audio_result = await audio_handler.process(audio_input)
            audio_text = audio_result.transcription
            print(f"[Chat API] Audio transcription: {audio_text}")
        except Exception as e:
            print(f"[ERROR] [Chat API] Audio processing failed: {e}", flush=True)
    ...
    if mode == "risk":
        ...
        # 从 risk_assessment 获取 analysis（避免引用未定义的 final_analysis）
        risk_analysis = risk_assessment.get("analysis", "")
...
async def llm_chat_mode(
    ...
    multimodal_parts = []
    if image_desc and image_desc not in ["[图像描述占位符]", ""]:
        multimodal_parts.append(f"【用户发送了一张图片，图片内容描述如下】\n{image_desc}")
    if audio_text and audio_text.strip():
        multimodal_parts.append(f"【用户发送了一段语音，语音内容转录如下】\n{audio_text}")
    if multimodal_parts:
    ...
    if llm_client and llm_client.is_available:
        if mode == "chat":
            # 构建关键词检测上下文（用于风险判断）
            keyword_context = f"初步关键词检测结果：等级{risk_level}，关键词「{keyword_found}」" if keyword_found else ""

            system_prompt = f"""你叫SmartGuard，是一位专业的反诈安全助手，专门帮助用户识别和防范电信网络诈骗。
{keyword_context}

【输出格式 - 必须严格遵守，无论回复内容是安全还是有风险】
你必须在回复末尾输出结构化风险标签。格式如下，一行都不能少：

[RISK_ANALYSIS]
risk_level: <0-5的数字>
risk_type: <英文单词，如 normal/low_risk/medium_risk/high_risk/loan_fraud/police_impersonation/investment_fraud/pig_butchery 等>
analysis: <1-2句话中文风险分析>
[/RISK_ANALYSIS]

【风险等级参考】
- 0 = 安全（正常聊天、问候、反诈宣传语）
- 1 = 低风险（偶尔提到钱但无异常）
- 2 = 中等风险（频繁提到贷款/转账/验证码等敏感操作）
- 3 = 较高风险（要求转账、提供密码、点击陌生链接）
- 4 = 高风险（恐吓威胁、冒充公检法）
- 5 = 极高风险（明确诈骗特征）

【risk_type 常用类型】
normal / low_risk / medium_risk / high_risk / loan_fraud / police_impersonation / investment_fraud / pig_butchery / telecom_fraud / ai_scam
...
risk_level: 0
risk_type: normal
analysis: 用户仅问候，无任何风险内容
[/RISK_ANALYSIS]

示例2（贷款诈骗）：
这段话听起来很可疑！对方以"低息贷款"为诱饵，要求你点击陌生链接填写资料，这是典型的虚假贷款诈骗套路。千万不要点击链接或提供任何个人信息！

[RISK_ANALYSIS]
risk_level: 3
risk_type: loan_fraud
analysis: 以低息贷款为诱饵要求点击陌生链接填写资料，疑似虚假贷款诈骗
[/RISK_ANALYSIS]

请严格按照以上格式回复，结构化标签部分不要换行、不要缺少任何一行、不要添加额外说明。"""
        else:
...
用通俗易懂的语言，让用户听完就能记住！"""
        ...
        try:
            ...
            # 预清理：去除意外残留的结构化标签（兜底处理）
            clean_response = re.sub(r'\[RISK_ANALYSIS\].*?\[/RISK_ANALYSIS\]', '', raw_response, flags=re.DOTALL).strip()
            clean_response = re.sub(r'\[/RISK_ANALYSIS\]', '', clean_response).strip()
            clean_response = re.sub(r'\[RISK_ANALYSIS\]', '', clean_response).strip()

            # 解析LLM返回的风险分析标签
            # 支持多种格式变体：单行、多行、字段顺序变化、标签前后有换行
            risk_match = re.search(
                r'\[RISK_ANALYSIS\]\s*risk_level[:：]\s*(\d+)\s*risk_type[:：]\s*([\w_]+)\s*analysis[:：]\s*(.+?)\s*\[/RISK_ANALYSIS\]',
            ...
            if risk_match:
                ...
                # 无法解析完整的 [RISK_ANALYSIS]...[/RISK_ANALYSIS] 标签
                # 检查是否有裸的 risk_level: 格式（LLM 可能只返回了 risk_level）
                bare_match = re.search(r'risk_level[:：]\s*(\d+)', clean_response)
                if bare_match:
                    parsed_level = int(bare_match.group(1))
                    response_text = re.sub(r'risk_level[:：]\s*\d+.*', '', clean_response, flags=re.DOTALL).strip()
                    if parsed_level > 0:
                        # LLM 给了风险等级
                        final_risk_level = parsed_level
                        final_risk_type = _detect_risk_type(text_lower)
                        final_analysis = "LLM分析"
                    elif keyword_found:
                        # LLM 返回 0 但关键词检测到了风险，用关键词兜底
                        final_risk_level = risk_level
                        final_risk_type = _detect_risk_type(text_lower)
                        final_analysis = f"关键词检测辅助判断：等级{risk_level}，关键词「{keyword_found}」"
                    else:
                        final_risk_level = 0
                        final_risk_type = "normal"
...
async def serve_spa(path: str):
    ...
    index_path = os.path.join(_frontend_src, "index.html")
