...
async def get_guardian_urgent_alerts(
    ...
    # 筛选高风险预警（等级>=3）
    urgent_alerts = []
    for user_alerts in result.get("alerts_by_user", {}).values():
        for alert in user_alerts:
            ...
            if level >= 3:
                ...
                # 获取被监护人信息
                protected_user = alert.get("protected_user", {})
                
                urgent_alerts.append({
                    "alert_id": alert.get("id", ""),
                    "user_id": alert.get("user_id", ""),
                    "user_name": protected_user.get("name", protected_user.get("username", "未知用户")),
                    "risk_level": level,
