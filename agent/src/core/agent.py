...
class AntiFraudAgent:
        ...
        try:
            ...
            if context.get("relevant_cases"):
                ...
                for i, c in enumerate(context["relevant_cases"][:3]):
                    ...
                    if "neg" in source_lower:
                        fraud_label = "【诈骗案例】"
                    elif "pos" in source_lower:
                        fraud_label = "【正常对话参考】"
                    else:
                        fraud_label = "【反诈知识参考】"
    ...
    async def _llm_full_analysis(
        ...
        try:
            ...
            # 构建相似案例上下文（传递完整信息给LLM）
            context_parts = []
            if context.get("relevant_cases"):
                cases_lines = []
                for c in context["relevant_cases"][:5]:
                    title = c.get("title", "未知")
                    source = c.get("source", "未知来源")
                    content = c.get("content", "")
                    ...
                    meta_parts = []
                    if metadata:
                        if metadata.get("is_fraud") is not None:
                            meta_parts.append(f"是否诈骗: {'是' if metadata['is_fraud'] else '否'}")
                        if metadata.get("fraud_type"):
                            meta_parts.append(f"诈骗类型: {metadata['fraud_type']}")
                        if metadata.get("confidence") and metadata['confidence'] > 0:
                            meta_parts.append(f"置信度: {metadata['confidence']}")
                        if metadata.get("scene"):
                            meta_parts.append(f"通话场景: {metadata['scene']}")
                        if metadata.get("reason"):
                            meta_parts.append(f"判断理由: {metadata['reason']}")

                    meta_str = " | ".join(meta_parts) if meta_parts else ""

                    case_entry = f"""标题: {title}
{fraud_label}
{meta_str}
对话内容:
{content}"""
                    cases_lines.append(case_entry)
            ...
            # 直接生成自然语言响应，不走模板
            # 知识库案例上下文已经填充好了
            prompt = f"""你是专业反诈助手 SmartGuard，帮助用户分析可疑内容是否为诈骗。
...
【分析要求 - 请严格按以下步骤分析】
1. 对照知识库中的【诈骗案例】，分析当前内容的通话模式、话术套路是否属于同类诈骗手法
2. 如果知识库中有【正常对话】，判断当前内容是否属于正常通话模式
3. 综合以上给出你的判断

【风险等级】
- 0 = 安全（正常问候、反诈宣传、安全内容）
- 1 = 低风险（偶尔提到钱但无异常行为）
- 2 = 中等风险（频繁提到敏感操作但不涉及转账）
- 3 = 较高风险（要求转账、提供密码验证码、点击陌生链接）
- 4 = 高风险/紧急（冒充公检法恐吓、明确诈骗）

【风险类型】
normal / low_risk / medium_risk / high_risk / loan_fraud / police_impersonation / investment_fraud / pig_butchery / telecom_fraud / refund_scam / brush_single_fraud
...
示例输出（不要复制此示例的具体内容，只参考格式）：
对方要求转账到陌生账户，这是典型的冒充熟人诈骗手法，属于高危行为。

[RISK_ANALYSIS]
risk_level: 3
risk_type: telecom_fraud
analysis: 对方要求转账到陌生账户，符合冒充熟人诈骗的典型特征
suggestion: 立即停止转账操作，先通过官方渠道核实对方身份
[/RISK_ANALYSIS]"""
            ...
            if response_text and response_text.strip():
                ...
                # 尝试分离自然语言回复和标签部分
                # 优先查找 [RISK_ANALYSIS] 标签
                tag_start = response_text.find('[RISK_ANALYSIS]')
                use_wrapper_tags = (tag_start != -1)

                if use_wrapper_tags:
                    tag_end = response_text.find('[/RISK_ANALYSIS]')
                    if tag_end == -1:
                        # 找不到闭合标签，直接用标签前的部分作为回复
                        llm_response = response_text[:tag_start].strip()
                        tag_content = ""
                    else:
                        llm_response = response_text[:tag_start].strip()
                        tag_content = response_text[tag_start + len('[RISK_ANALYSIS]'):tag_end].strip()
                else:
                    # 没有包装标签，尝试查找裸的 risk_level: 标签
                    tag_start = response_text.find('risk_level:')
                    if tag_start == -1:
                        tag_start = response_text.find('risk_level：')  # 中文冒号

                    if tag_start != -1:
                        llm_response = response_text[:tag_start].strip()
                        tag_content = response_text[tag_start:].strip()
                    else:
                ...
                if tag_content:
                    lvl_m = re.search(r'risk_level[:：]\s*(\d)', tag_content)
                    if lvl_m:
                        rlvl = int(lvl_m.group(1))
                    type_m = re.search(r'risk_type[:：]\s*([\w_]+)', tag_content)
                    if type_m:
                        rtype = type_m.group(1)
                    analysis_m = re.search(r'analysis[:：]\s*(.+)', tag_content, re.DOTALL)
                    if analysis_m:
                        ranalysis = analysis_m.group(1).strip()
                    suggest_m = re.search(r'suggestion[:：]\s*(.+)', tag_content, re.DOTALL)
                    if suggest_m:
                        rsuggestion = suggest_m.group(1).strip()

                if not tag_content or (rlvl == 0 and kw_list):
                    # LLM未输出结构化标签，或输出了 risk_level:0（可能忽略了关键词）时，用关键词分析结果兜底
                    rlvl = keyword_analysis.risk_level
                    rtype = keyword_analysis.risk_type
                    ranalysis = keyword_analysis.analysis
                    rsuggestion = keyword_analysis.suggestion
                ...
                if llm_response and len(llm_response) > 5:
                    ...
                    return RiskAssessment(
                        ...
                        recommended_actions=[rsuggestion] if rsuggestion else [],
                        timestamp=time.time(),
                        _direct_response=llm_response
                    )
    ...
    async def _generate_response(
        ...
        if direct:
            # 兜底清理：确保即使LLM意外输出标签也不会泄露给用户
            cleaned = re.sub(r'\[RISK_ANALYSIS\].*?\[/RISK_ANALYSIS\]', '', direct, flags=re.DOTALL).strip()
            cleaned = re.sub(r'\[/RISK_ANALYSIS\]', '', cleaned).strip()
            cleaned = re.sub(r'\[RISK_ANALYSIS\]', '', cleaned).strip()
            return cleaned
