    ...
    def _process_tele_fraud_record(self, data: Dict, path: Path, line_idx: int):
        ...
        for msg in messages:
            ...
            if role == 'user':
                ...
                if text and len(text) > 20:  # 过滤太短的
                    user_content = text
                    conversation_parts.append(f"通话内容：{text[:800]}")
            elif role == 'assistant':
                # 提取助手回复中的分析结果
                text = self._extract_assistant_content(content)
                if text and len(text) > 10:  # 过滤太短的
                    assistant_response = text
                    conversation_parts.append(f"分析结果：{text[:1000]}")
        ...
        if conversation_parts:
            ...
            # 生成标题
            title = "电话诈骗案例"
            content_lower = user_content.lower() + assistant_response.lower()
            if "公安" in content_lower or "警方" in content_lower or "法院" in content_lower:
                title = "冒充公检法诈骗"
            elif "投资" in content_lower or "理财" in content_lower or "收益" in content_lower:
                title = "投资理财诈骗"
            elif "刷单" in content_lower or "兼职" in content_lower:
                title = "刷单兼职诈骗"
            elif "客服" in content_lower or "退款" in content_lower:
                title = "客服退款诈骗"
            elif "贷款" in content_lower or "征信" in content_lower:
                title = "贷款征信诈骗"
            
            doc = KnowledgeDocument(
                content=full_text[:15000],  # 限制长度
                source=str(path.relative_to(self.base_path.parent)),
                doc_type="jsonl",
                title=title,
                metadata={
